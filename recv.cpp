...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
    ...
    if(shmid < 0)
    {
        perror("Error occurred from shmget");
        exit(-1);
    }
    ...
    if(msqid < 0)
    {
        perror("Error occurred from  msgget");
        exit(-1);
    }
...
unsigned long mainLoop(const char* fileName)
{
    ...
    while(msgSize != 0)
    {
        ...
        {
            perror(" Error occurred from msgsnd");
            fclose(fp);
            exit(-1);
        }
        ...
        cout << "== The number of bytes received is == " << msgSize << endl;
        ...
        if(msgSize != 0)
        {
            ...
            fputs((char*) sharedMemPtr, fp);

            
//            if(fwrite(sharedMemPtr, sizeof(char), msgSize, fp) < 0)
//            {
//                perror("Error occurred from   fwrite");
//            }
//
...
void cleanUp(const int& shmid, const int& msqid, void* sharedMemPtr)
{
    ...
    {
        perror("Error occurred from shmdt");
        exit(-1);
    }
    /* TODO: Deallocate the shared memory segment */
    if(shmctl(shmid, IPC_RMID, NULL) < 0)
    {
        perror("Error occurred from shmctl");
        exit(-1);
    }
    /* TODO: Deallocate the message queue */
    if(msgctl(msqid, IPC_RMID, NULL) < 0)
    {
        perror("Error occurred from msgctl");
        exit(-1);
    }
}
